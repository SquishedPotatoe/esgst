...
class GeneralMultiManager extends Module {
	...
	mm_openPopout(obj, items, itemsKey) {
		...
		Object.keys(obj.checkboxes).forEach((key, i) => {
			...
			let toggleSwitch = new ToggleSwitch(
				...
				false,
				null,
				Settings.get(`mm_enable${key}`)
			);
			toggleSwitch.onEnabled = this.mm_enable.bind(this, obj, itemsKey === key ? items : null, key);
			toggleSwitch.onDisabled = this.mm_disable.bind(
				this,
				obj,
				itemsKey === key ? items : null,
				key
			);
			this.mm_setSection(
				obj,
	...
	mm_enable(obj, items, key) {
		...
		items.forEach((item) => {
			let checkbox =
				getChildByClassName(item.innerWrap, 'esgst-mm-checkbox') ||
				getChildByClassName(item.innerWrap.parentElement, 'esgst-mm-checkbox');
			if (checkbox) return;
			...
			if (event.shiftKey) {
				...
				for (const element of elements) {
					...
					const currentItem = items.filter(
						(subItem) => element === (subItem[currentKey] && subItem[currentKey].checkbox)
					)[0];
					if (currentItem) {
	...
	mm_exportLinks(obj, items, key) {
		...
		items.forEach((item) => {
			if (
				...
				return;
			links.push(
				`[${escapeMarkdown(item.name || item.title || item.code)}](https://${
					key === 'Games'
						? `store.steampowered.com/${item.type.slice(0, -1)}/${item.code}`
						: `${window.location.hostname}/${key.toLowerCase().slice(0, -1)}/${item.code}/`
				})`
			);
		});
		...
		switch (key) {
				...
				items.forEach((item) => {
					...
					links.push(
						line
							...
							.replace(/\[COMMENTS]/gi, item.comments)
							.replace(/\[COPIES]/gi, item.copies)
							.replace(/\[CREATOR]/gi, item.creator)
							.replace(/\[END-TIME="(.+?)"]/gi, this.mm_formatDate.bind(this, item.endTime))
							.replace(/\[ENTRIES]/gi, item.entries)
							.replace(/\[GROUPS]/gi, (item.groupNames || []).join(`, `))
							.replace(/\[LEVEL]/gi, item.level)
							.replace(/\[NAME]/gi, escapeMarkdown(item.name))
							.replace(/\[POINTS]/gi, item.points)
							.replace(/\[SHORT-URL]/gi, `https://www.steamgifts.com/giveaway/${item.code}/`)
							.replace(/\[START-TIME="(.+?)"]/gi, this.mm_formatDate.bind(this, item.startTime))
							.replace(/\[STEAM-ID]/gi, item.id)
							.replace(/\[STEAM-TYPE]/gi, item.type.slice(0, -1))
							.replace(
								/\[STEAM-URL]/gi,
								`http://store.steampowered.com/${item.type.slice(0, -1)}/${item.id}`
							)
							...
							.replace(/\[WINNERS]/gi, winners.map((x) => x.username).join(`, `))
							.replace(
								/\[WINNERS-STATUS]/gi,
								winners.map((x) => `${x.username} (${x.status})`).join(`, `)
							)
					);
				});
				break;
				...
				items.forEach((item) => {
					...
					links.push(
						line
							...
							.replace(/\[CATEGORY]/gi, escapeMarkdown(item.category))
							.replace(/\[CODE]/gi, item.code)
							.replace(/\[COMMENTS]/gi, item.comments)
							.replace(
								/\[CREATED-TIME="(.+?)"]/gi,
								this.mm_formatDate.bind(this, item.createdTimestamp)
							)
							.replace(/\[POLL]/gi, item.poll ? 'Yes' : 'No')
							.replace(/\[SHORT-URL]/gi, `https://www.steamgifts.com/discussion/${item.code}/`)
							.replace(/\[TITLE]/gi, escapeMarkdown(item.title))
							.replace(
								/\[URL]/gi,
								`https://www.steamgifts.com${item.url.match(/\/discussion\/.+/)[0]}`
							)
					);
				});
				break;
			case 'Users':
				items.forEach((item) => {
					...
					links.push(
						line
							.replace(/\[URL]/gi, `https://${window.location.hostname}/user/${item.code}`)
							.replace(/\[USERNAME]/gi, item.code)
							.replace(/\[SENTCOUNT]/gi, item.sentCount)
							.replace(/\[SENTVALUE]/gi, item.sentValue)
							.replace(/\[RECEIVEDCOUNT]/gi, item.receivedCount)
							.replace(/\[RECEIVEDVALUE]/gi, item.receivedValue)
							.replace(/\[GIFTDIFFERENCE]/gi, item.giftDifference)
							.replace(/\[VALUEDIFFERENCE]/gi, item.valueDifference)
					);
				});
				break;
			case 'Games':
				items.forEach((item) => {
					...
					links.push(
						line
							.replace(/\[ID]/gi, item.code)
							.replace(/\[NAME]/gi, escapeMarkdown(item.name))
							.replace(/\[TYPE]/gi, item.type.slice(0, -1))
							.replace(
								/\[URL]/gi,
								`https://store.steampowered.com/${item.type.slice(0, -1)}/${item.code}`
							)
					);
				});
				break;
				...
				items.forEach((item) => {
					...
					links.push(
						line
							.replace(/\[CODE]/gi, item.code)
							.replace(/\[NAME]/gi, escapeMarkdown(item.name))
							.replace(/\[URL]/gi, `https://www.steamgifts.com/group/${item.code}/`)
					);
				});
				break;
			default:
	...
	async mm_getSearchReplaceUrlRequest(obj, details, response, responseHtml) {
		...
		if (description) {
			let match = Settings.get('mm_useRegExp')
				? description.value.match(searchValue)
	...
	mm_calculateGiveaways(obj, items) {
		...
		if (points > Session.counters.points) {
			obj.textAreaGiveaways.value = `You will need to wait ${this.esgst.modules.giveawaysTimeToEnterCalculator.ttec_getTime(
				Math.round((nextRefresh + 15 * Math.floor((points - Session.counters.points) / 6)) * 100) /
					100
			)} to enter all selected giveaways for a total of ${points}P.${
				points > 400
	...
	mm_exportEncryptedGiveaways(obj, items) {
		...
		items.forEach((item) => {
			if (
				...
				return;
			encrypted.push(
				`[](ESGST-${this.esgst.modules.giveawaysGiveawayEncrypterDecrypter.ged_encryptCode(
					item.code
				)})`
			);
		});
	...
	mm_selectGlwcUsers(obj, items) {
		...
		Tabs.open(
			`https://www.steamgifts.com/account/settings/profile?esgst=glwc&users=${users.join(',')}`
		);
	}
