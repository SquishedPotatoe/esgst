...
class GiveawaysDeleteKeyConfirmation extends Module {
	...
	dkc_getLinks(context, main, source, endless) {
		const elements = context.querySelectorAll(
			`${
				endless
					? `.esgst-es-page-${endless} .form__key-btn-delete, .esgst-es-page-${endless}.form__key-btn-delete`
					: '.form__key-btn-delete'
			}`
		);
		for (let i = elements.length - 1; i > -1; --i) {
			...
			newElement.addEventListener(
				'click',
				createConfirmation.bind(
					common,
					'Are you sure you want to delete this key?',
					this.dkc_deleteKey.bind(createConfirmation, newElement),
					null
				)
			);
		}
